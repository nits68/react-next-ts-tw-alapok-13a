...
export default function AligmentBar(props: Props) {
  return (
    <nav className="flex flex-wrap justify-center gap-3 bg-green-300 p-4">
      ...
      {props.aligments.map((align) => (
        <button
          key={align.value}
          className={`btn btn-primary ${props.aktValue === align.value ? 'bg-blue-500 text-white shadow-lg' : 'bg-white text-gray-700 shadow hover:bg-gray-100'}`}
